...
### Protocol Example  
- Master Protocol Example  
<img src="image-6.png" width="60%"/>  
- Slave protocol Example(RTC - M41T93)  
<img src="slave_protocol_example.png" width="60%"/>  
    > CS할당(E = 0)이후 ADDR 수신, 그다음부터는 다음ADDR에 들어온 데이터 저장. CS가 해제(E = 1)될 때까지 반복. MASTER에서 READ하려면 dummy data를 전송해야한다. 해당 모듈은 첫번째 데이터(주소데이터)에 READ/WRITE선택 비트를 하나 더 추가했다. w/r비트까지 9비트를 수신하는 구조이다. 이 구조는 설계하기 나름이다. 첫번째 데이터를 통째로 r/d신호로 쓰기도 하고 r/w둘 다 동시에 가능하게 구현할 수도 있다.  
